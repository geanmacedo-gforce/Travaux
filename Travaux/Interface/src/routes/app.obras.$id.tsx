...
function ObraDetail() {
  ...
  const totalMO = horas.reduce((s:number,h:any)=>s+Number(h.valor_total),0);
  const totalMat = mats.reduce((s:number,m:any)=>s+Number(m.valor_total),0);
  const cat = (c: string) => desp.filter((d:any)=>d.categoria===c).reduce((s:number,d:any)=>s+Number(d.valor),0);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-4 gap-3">
        <Mini label={t("Faturado")} value={fmtBRL(obra.valor_contratado)}/>
      ...
      <Tabs defaultValue="horas">
        <TabsList>
          <TabsTrigger value="horas">{t("Horas")}</TabsTrigger>
          <TabsTrigger value="materiais">{t("Produtos & Insumos")}</TabsTrigger>
          <TabsTrigger value="funcionarios">{t("Funcionários")}</TabsTrigger>
          <TabsTrigger value="despesas">{t("Despesas")}</TabsTrigger>
          {divs.length > 0 && (
            <TabsTrigger value="divergencias" className="gap-1">
              <AlertTriangle className="h-3.5 w-3.5 text-warning" />
              {t("Divergências")} ({divs.length})
            </TabsTrigger>
          )}
        </TabsList>

        <TabsContent value="horas">
          <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle>{t("Horas trabalhadas") || "Worked hours"}</CardTitle><NewButton onClick={openHNew}/></CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow><TableHead>{t("Data")}</TableHead><TableHead>{t("Funcionário")}</TableHead><TableHead>{t("Horas")}</TableHead><TableHead>{t("Valor")}</TableHead><TableHead>{t("Descrição")}</TableHead><TableHead></TableHead></TableRow></TableHeader>
                <TableBody>{horas.map((h:any)=>(<TableRow key={h.id}><TableCell>{fmtDate(h.data)}</TableCell><TableCell>{h.funcionario_nome}</TableCell><TableCell>{fmtHours(h.horas)}</TableCell><TableCell>{fmtBRL(h.valor_total)}</TableCell><TableCell className="text-xs text-muted-foreground">{h.descricao}</TableCell><TableCell><ConfirmDelete onConfirm={()=>delHoras(h.id)}/></TableCell></TableRow>))}
                {horas.length===0 && <TableRow><TableCell colSpan={6} className="text-center py-6 text-muted-foreground">{t("Sem horas registradas.") || "No recorded hours."}</TableCell></TableRow>}
                </TableBody>
              </Table>
            </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="materiais">
          <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle>{t("Produtos & Insumos")}</CardTitle><NewButton onClick={()=>setMOpen(true)}/></CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow><TableHead>{t("Data")}</TableHead><TableHead>{t("Produto") || "Product"}</TableHead><TableHead>{t("Qtd") || "Qty"}</TableHead><TableHead>{t("Unit.") || "Unit"}</TableHead><TableHead>{t("Total") || "Total"}</TableHead><TableHead>{t("Comprovante")}</TableHead><TableHead>Link</TableHead><TableHead></TableHead></TableRow></TableHeader>
                <TableBody>{mats.map((m:any)=>(
                  <TableRow key={m.id}>
                    <TableCell>{fmtDate(m.data)}</TableCell>
                    <TableCell>{m.produto_nome}</TableCell>
                    <TableCell>{Number(m.quantidade)} {m.produto_unidade}</TableCell>
                    <TableCell>{fmtBRL(m.valor_unitario)}</TableCell>
                    <TableCell>{fmtBRL(m.valor_total)}</TableCell>
                    <TableCell><ComprovanteThumb path={m.comprovante_url}/></TableCell>
                    <TableCell>{m.link_url ? <a href={m.link_url} target="_blank" rel="noreferrer" className="text-primary underline text-xs">{t("abrir") || "open"}</a> : <span className="text-muted-foreground text-xs">—</span>}</TableCell>
                    <TableCell><ConfirmDelete onConfirm={()=>delMat(m.id)}/></TableCell>
                  </TableRow>
                ))}
                {mats.length===0 && <TableRow><TableCell colSpan={8} className="text-center py-6 text-muted-foreground">{t("Sem materiais.") || "No materials."}</TableCell></TableRow>}
                </TableBody>
              </Table>
            </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="funcionarios">
          <Card><CardHeader><CardTitle>{t("Funcionários desta obra") || "Employees on this project"}</CardTitle></CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow><TableHead>{t("Funcionário")}</TableHead><TableHead>{t("Função")}</TableHead><TableHead>{t("Total horas")}</TableHead><TableHead>{t("Total pago") || "Total paid"}</TableHead></TableRow></TableHeader>
                <TableBody>
                  {(() => {
                    const map = new Map<string, { nome: string; funcao?: string; horas: number; valor: number }>();
                    horas.forEach((h: any) => {
                      const cur = map.get(h.funcionario_id) ?? { nome: h.funcionario_nome ?? "—", horas: 0, valor: 0 };
                      cur.horas += Number(h.horas);
                      cur.valor += Number(h.valor_total);
                      map.set(h.funcionario_id, cur);
                    });
                    map.forEach((v, k) => { const f = funcs.find((x: any) => x.id === k); if (f) v.funcao = f.funcao; });
                    const rows = Array.from(map.entries());
                    if (rows.length === 0) return <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">{t("Nenhum funcionário registrou horas.") || "No employee has recorded hours."}</TableCell></TableRow>;
                    return rows.map(([k, v]) => (
                      <TableRow key={k}>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-warning">
                  ...
                  {t("Check-ins fora do raio da obra")}
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t("Data/Hora")}</TableHead>
                      <TableHead>{t("Funcionário")}</TableHead>
                      <TableHead>{t("Distância")}</TableHead>
                      <TableHead>{t("Raio permitido")}</TableHead>
                      <TableHead>{t("Desvio")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {divs.map((d: any) => (
                      <TableRow key={d.id}>
                        ...
                        <TableCell>{d.funcionario_nome ?? "—"}</TableCell>
                        <TableCell>{d.distancia_metros >= 1000 ? `${(d.distancia_metros / 1000).toFixed(2).replace(".",",")} km` : `${d.distancia_metros} m`}</TableCell>
                        <TableCell>{d.raio_metros >= 1000 ? `${(d.raio_metros / 1000).toFixed(2).replace(".",",")} km` : `${d.raio_metros} m`}</TableCell>
                        <TableCell className="text-destructive font-medium">{d.desvio_metros >= 1000 ? `+${(d.desvio_metros / 1000).toFixed(2).replace(".",",")} km` : `+${d.desvio_metros} m`}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
